...
# ── Default hyperparameters (Optuna-tuned, stored in config.py) ───────────────
DEFAULT_PARAMS: dict = {**XGB_PARAMS, "random_state": RANDOM_STATE}
...
def make_symmetric(df: pd.DataFrame) -> pd.DataFrame:
    """
    Flip Red ↔ Blue to double the dataset and remove corner bias.
...
def cross_validate(df: pd.DataFrame, params: dict | None = None, n_splits: int = 5) -> list[float]:
    ...
    # Remove keys that XGBClassifier doesn't accept as constructor args
...
def tune_hyperparameters(df: pd.DataFrame, n_trials: int = 50) -> dict:
    """
    Use Optuna to find the best XGBoost hyperparameters via TimeSeriesSplit CV.

    Returns the best parameter dict to pass directly to XGBClassifier.
    """
    try:
        import optuna
...
def plot_model_performance(
    ...
    roc_auc = auc(fpr, tpr)
    ax2.plot(fpr, tpr, color="darkgreen", lw=2, label=f"ROC (AUC = {roc_auc:.3f})")
...
def main(tune: bool = False, n_trials: int = 50) -> None:
    ...
    # Build final model params (add early stopping for the final fit)
    fit_params = {k: v for k, v in best_params.items() if k != "eval_metric"}
    model = XGBClassifier(
        **fit_params,
        eval_metric="logloss",
    ...
    log.info("Saving model artifacts to %s…", MODEL_XGB_PATH.parent)
    joblib.dump(model,         MODEL_XGB_PATH)
    joblib.dump(feature_names, MODEL_XGB_FEATURES)
    log.info("Saved: %s", MODEL_XGB_PATH.name)
    log.info("Saved: %s", MODEL_XGB_FEATURES.name)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train XGBoost UFC fight predictor.")
    parser.add_argument(
        "--tune",
        action="store_true",
