"""
random_forest.py -- Random Forest model for UFC fight prediction.
...
DEFAULT_PARAMS: dict = {**RF_PARAMS, "random_state": RANDOM_STATE}
...
def tune_hyperparameters(df: pd.DataFrame, n_trials: int = 50) -> dict:
    """Use Optuna to find the best Random Forest hyperparameters via TimeSeriesSplit CV."""
    try:
        import optuna
...
def main(tune: bool = False, n_trials: int = 50) -> None:
    ...
    log.info("Saving model artifacts to %s...", MODEL_RF_PATH.parent)
    joblib.dump(model,         MODEL_RF_PATH)
    joblib.dump(feature_names, MODEL_RF_FEATURES)
    log.info("Saved: %s", MODEL_RF_PATH.name)
    log.info("Saved: %s", MODEL_RF_FEATURES.name)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Random Forest UFC fight predictor.")
    parser.add_argument(
        "--tune",
        action="store_true",
