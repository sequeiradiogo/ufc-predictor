...
- **`fight_stats.total_fight_time` is cumulative pre-fight time, not a per-fight duration -- any code reading it must treat it that way**: since commit `a33e19e` (2026-07-01), `rolling.py` writes this column as the seconds fought *before* each fight (0 on debut), not that fight's own duration. `predict.py`'s `compute_live_career_stats()`/`compute_ewma_stats_single()` were written before that commit and, until fixed (2026-07-24), still re-summed it as if each row were an independent per-fight value -- a double-cumulation bug that silently corrupted `splm`/`sapm`/`td_avg` and every EWMA/slope/opp-adjusted feature derived from them (11 total) whenever run against a DB with the corrected schema. It stayed invisible for weeks because the CI-downloaded release DB was stale (pre-dated the schema change) and happened to still match the old, buggy assumption -- a locally-rebuilt DB exposed it immediately. If you touch either function again, remember: `total_fight_time` for a fight = get that fight's own duration from `match_time_sec + (finish_round-1)*300` (via the `fights` table), never from summing `total_fight_time` itself across rows.
  - **This same column went stale again on 2026-07-28, for a different reason: the code fix was merged but a full `rolling.py` rebuild was never actually re-run against the shared release DB.** Before `a33e19e`, `total_fight_time` was in `rolling.py`'s upsert exclusion list (`_SCRAPER_COLS`), so it was *never overwritten* -- it stayed frozen at whatever the scraper originally wrote (`scrapers/ufcstats.py`'s `total_secs`, i.e. that single fight's own duration, not a running total). `a33e19e` removed it from the exclusion list, but that only fixes rows the next time `rolling.py` actually runs a full rebuild against them -- and one apparently never happened after the commit merged, so 97% of rows (16,976/17,474) in the `data-artifacts-latest` release DB were still stuck on the pre-fix per-fight value as of 2026-07-28, discovered when it produced impossible live-inference values (`splm` = 80.9 strikes/min for one fighter; a sane value is ~2-8). **Detection**: `SELECT MAX(total_fight_time) FROM fight_stats` -- if it's capped at ~1500 (5 rounds x 300s, the max possible single-fight duration) across the whole table, the column was never actually rebuilt post-fix, no matter what the code says it *should* do. **Fix**: re-run the full DB rebuild (`python -c "from db.rolling import main; from config import DB_UFCSTATS_PATH; main(db_path=DB_UFCSTATS_PATH)"`), verify `MAX(total_fight_time)` now spans into the thousands/tens-of-thousands of seconds, then re-upload via `gh release upload data-artifacts-latest db/ufc_ufcstats.db --clobber`. Note `rolling.py`'s own `splm`/`sapm`/`td_avg` computations are self-correcting on every run regardless (they recompute a fresh in-memory cumulative time from raw `match_time_sec`/`finish_round` rather than trusting the stored column, per `_compute_time_dependent()`) -- only the *persisted* `total_fight_time` column itself, and anything that reads it directly like `predict.py`'s live-inference path, can go stale this way.
  - **A local `db/*.db` file can silently diverge from the release asset even when its size and reported timestamp match** -- a prior session's local testing/scraping can mutate the working copy in place. Before trusting a local repro to represent CI behavior (or vice versa), compare `sha256sum` of the local file against the release asset's digest (`gh release view data-artifacts-latest --json assets`), not just size/date.
- **`requirements.txt` pins the ML stack (scikit-learn/xgboost/lightgbm/torch/numpy/pandas) to exact versions, not `>=`** -- they must match whatever versions `models_v1(_prod)/*.joblib` were actually trained/pickled with. An unpinned install resolves to whatever's newest at CI run time; confirmed via `InconsistentVersionWarning` on every calibrated component (IsotonicRegression, LogisticRegression, StandardScaler, RandomForest, LabelEncoder) that a version drift there silently corrupts calibrated ensemble probabilities on load. After any retrain that bumps a library version locally, update these pins to match and re-verify predictions look sane in CI before trusting them.
- **Live-inference feature parity is not enforced anywhere -- a feature can go silently dead**: `build_feature_vector()`'s fallback for any `<stat>_diff` feature not explicitly handled is `red_stats.get(<stat>, 0) - blue_stats.get(<stat>, 0)`. If `compute_live_career_stats()` (or another `extra_r`/`extra_b` source) never populates that key, the feature computes as `0` for literally every prediction with no error. This is how `height_diff`/`reach_diff` went dead, and a full audit on 2026-07-24 found **22 features this way** -- 26.6% of the XGBoost model's total feature importance (`age_ratio_diff` alone was the #2 most important feature in the model and was contributing nothing): opponent-adjusted stats (`opp_adj_*`, 5), zone defense (`head_def_diff`/`body_def_diff`/`dist_def_diff`/`ground_def_diff`), round-1 EWMA (`ewma_ctrl_r1_diff`/`ewma_splm_r1_diff`/`ewma_reversals_r1_diff`), EWMA zone/output-rate extras (`ewma_head_acc_diff`/`ewma_body_acc_diff`/`ewma_dist_acc_diff`/`ewma_clinch_per_diff`/`ewma_sub_att_diff`/`ewma_kd_received_diff`/`ewma_reversals_diff`), `career_reversals_diff`, `age_ratio_diff`, `ufc_experience_diff`. All fixed 2026-07-24 by extending `compute_live_career_stats()` (height/reach/age), `compute_ewma_stats_single()` (zone acc + output-rate extras), and two new functions -- `compute_r1_ewma_stats()` (round-1 EWMA, guarded on `fight_stats_rounds` existing) and `compute_opp_adjusted_stats()` (opponent-adjusted stats, with a module-level cached league-average query). When adding any new trained feature, verify it's actually populated at inference -- don't assume the fallback will fail loudly; it won't.
