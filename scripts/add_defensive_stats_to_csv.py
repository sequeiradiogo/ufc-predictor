...
def build_lookup(conn: sqlite3.Connection) -> dict[str, pd.DataFrame]:
    """Return {name_lower: DataFrame(date, sapm, str_def, td_def)} from UFCStats."""
...
def _lookup(name: str, fight_date: pd.Timestamp,
            lookup: dict[str, pd.DataFrame]) -> tuple[float, float, float]:
    """Return (sapm, str_def, td_def) pre-fight for fighter name."""
    ...
    row = before.iloc[-1]
    return (
        float(row["sapm"])    if pd.notna(row["sapm"])    else np.nan,
        float(row["str_def"]) if pd.notna(row["str_def"]) else np.nan,
        float(row["td_def"])  if pd.notna(row["td_def"])  else np.nan,
    )
...
def main(dry_run: bool = False) -> None:
    ...
    r_sapm = np.full(len(df), np.nan)
    r_str_def = np.full(len(df), np.nan)
    r_td_def = np.full(len(df), np.nan)
    b_sapm = np.full(len(df), np.nan)
    b_str_def = np.full(len(df), np.nan)
    b_td_def = np.full(len(df), np.nan)

    matched = no_match = 0
    for i, row in enumerate(df.itertuples(index=False)):
        rs, rsd, rtd = _lookup(row.R_fighter, row.date, lookup)
        bs, bsd, btd = _lookup(row.B_fighter, row.date, lookup)
    ...
    print("  Added: R_sapm, B_sapm, R_str_def, B_str_def, R_td_def, B_td_def")
